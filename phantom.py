...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.cbSize.currentTextChanged.connect(
            self.cbSize_currentTextChanged)
        self.ui.cbProperty.currentTextChanged.connect(
            self.cbProperty_currentTextChanged)
    ...
    def pbBrowse_clicked(self):
        ...
        self.fileName, _filter = QFileDialog.getOpenFileName(
            self, "Open", "", "Filter -- All Files (*);;Python Files (*.py)")
    ...
    def getPos(self, event):
        size = int(self.ui.cbSize.currentText())
        x = math.floor(event.pos().x()*size /
                       self.ui.lblPhantom.frameGeometry().width())
        y = math.floor(event.pos().y()*size /
                       self.ui.lblPhantom.frameGeometry().height())
        t1 = self.cvImgT1[y][x]
        t2 = self.cvImgT2[y][x]
        print('t1:', t1, ' ', 't2:', t2)
        fa = np.deg2rad(self.ui.sbFA.value())
        color = [QColor(0, 0, 255, 75), QColor(0, 255, 0, 75), QColor(
            255, 0, 0, 75), QColor(255, 0, 255, 75), QColor(255, 255, 0, 75)]
        if self.number % 5 == 0:
            print('1')
            self.highlight.end()
        ...
        print(x, 'x', y)
        self.highlight.fillRect(
            self.position[self.number % 5][0], self.position[self.number % 5][1], 10, 10, color[self.number % 5])
        self.ui.lblPhantom.setPixmap(self.pixmap)
        self.plt = self.ui.gvDecay
        self.graph(lambda x: (np.exp(-x/t2)*(np.sin(fa))),
                   (0, 10000, 0.01), self.plt)
        self.plt = self.ui.gvRecovery
        self.graph(lambda x: (np.exp(-x/t1)*((np.cos(fa))-1)+1),
                   (0, 10000, 0.01), self.plt)
        self.number += 1
    ...
    def winResize(self):
        # print('resize');
        print('label:', self.ui.lblPhantom.frameGeometry().width(),
              'x', self.ui.lblPhantom.frameGeometry().height())
        print('Window:', self.frameGeometry().width(),
              'x', self.frameGeometry().height())
        '''scale = self.frameGeometry().height()-400+128
        try:
            self.pixmap = self.pixmap.scaled(
                int(scale), int(scale), QtCore.Qt.KeepAspectRatio)
